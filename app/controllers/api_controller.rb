class ApiController < ApplicationController
  ...
  def messages
    ...
    if params[:codes].present?
      ...
      @messages =   Message.published.not_deleted.includes(:photo_files, :school, :author).for_app.by_group_or_student_ids_or_recipients(groups_ids, student_ids).order(updated_at: :desc).limit(30) #.includes(:mfiles)
      ...
      @messages_with_students = @messages.map { |m|
        ...
        if m.school.allow_translation
          ...
          m.content << "<div><a href='" + url + "' type='button' class='btn' style='display: inline-block;"\
