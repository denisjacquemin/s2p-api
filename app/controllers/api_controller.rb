class ApiController < ApplicationController
  ...
  def messages
    ...
    if params[:codes].present?
      ...
      @messages =   Message.published.not_deleted.includes(:photo_files, :school, :author).for_app.by_group_or_student_ids_or_recipients(groups_ids, student_ids).order(updated_at: :desc).limit(40) #.includes(:mfiles)
