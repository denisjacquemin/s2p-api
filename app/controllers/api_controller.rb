      ...
      @messages_with_students = @messages.map { |m|
        ...
        if m.school.allow_translation
          ...
          m.content << "<a href='" + url + "' type='button' class='btn' style='display: inline-block;"\
            ...
            "border-radius: .25rem;transition: color .15s ease-in-out,background-color .15s ease-in-out,border-color .15s ease-in-out,box-shadow .15s ease-in-out;color: #fff;background-color: #6c757d;border-color: #6c757d;overflow: visible;    padding: .25rem .5rem;font-size: .875rem;line-height: 1.5;border-radius: .2rem;'>Traduire</a>"
        end
